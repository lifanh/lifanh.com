...
export default function ZeoSevenFonts(): FontProvider {
  return {
    name: "ZeoSeven Fonts",
    resolveFont: async ({ familyName }) => {
      const id = ZEOSEVEN_FONTS[familyName as keyof typeof ZEOSEVEN_FONTS];
      if (!id) return { fonts: [] };
                ...
                case "src": {
                  ...
                  children.forEach((node, index) => {
                    if (node.type === "Function") {
                      if (node.name === "local" && node.children.first?.type === "String") font.src.push({ name: node.children.first.value });
